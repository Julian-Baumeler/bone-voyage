/**
 * OpenGEM — simplified: drop CT → optional 1-stroke paint → Generate → 3D
 */
...
async function loadBoneLabels() {
  try {
    const res = await fetch("/api/session/bones-labels");
...
async function loadCtVolume() {
  ...
  const res = await fetch("/api/session/ct-volume");
...
api("/api/health")
  .then(async (h) => {
    setStatus(`OpenGEM ${h.version} · drop a CT to auto-split bones`, "ok");
    setDebug(
